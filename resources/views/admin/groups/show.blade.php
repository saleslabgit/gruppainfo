...
<div class="ui-detail-stack">
    @if(in_array($group->status, [App\Domain\Group\GroupStatus::Draft, App\Domain\Group\GroupStatus::AwaitingPayment], true) && $group->successfulNonRefundedPayments->isNotEmpty())
        <x-ui.alert variant="warning" title="Удаление группы заблокировано">
            Сначала верните успешный платёж во внешней платёжной системе и зафиксируйте возврат.
            @include('admin.groups._payment-facts', ['payments' => $group->successfulNonRefundedPayments])
        </x-ui.alert>
    @endif
