@props(['name', 'id' => null, 'value' => '', 'placeholder' => 'Выберите значение', 'options' => [], 'disabled' => false, 'error' => false])
@php($controlId = $id ?? $name)
<div class="ui-select" data-ui-select>
    <input type="hidden" name="{{ $name }}" value="{{ $value }}" data-ui-select-value>
    <button id="{{ $controlId }}" type="button" aria-haspopup="listbox" aria-expanded="false" @disabled($disabled) {{ $attributes->class(['ui-select__trigger', 'is-placeholder' => $value === '', 'is-invalid' => $error]) }} data-ui-select-trigger><span data-ui-select-label>{{ $options[$value] ?? $placeholder }}</span><x-ui.icon name="chevron-down" size="16" /></button>
