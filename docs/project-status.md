...
Текущий реализованный этап: **Stage 6 — кабинет психолога**. Финальная продуктовая/визуальная приёмка остаётся за пользователем.
...
- `/cabinet/groups` с truthful empty state без преждевременного Stage 7 UI;
...
Последующая продуктовая функциональность ещё не реализована. Пока нет CRUD и реального списка групп, обработки заявок, scheduler-задач, публичного HTTP API, email-onboarding, SMTP-интеграции, платёжного адаптера, банковского webhook-контроллера и внешних интеграций. Раздел **Мои группы** намеренно остаётся пустой продуктовой поверхностью до Stage 7.
...
Следующий этап по актуальному `SPEC.md` — **Stage 7: CRUD групп и модерация**.

Stage 7 должен наполнить существующий раздел **Мои группы** реальным ownership-защищённым CRUD и moderation workflow, не добавляя публичную анкету, `/api/v1`, password setup email, SMTP или платежи раньше утверждённых этапов.
