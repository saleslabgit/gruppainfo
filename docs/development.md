...
Seed идемпотентен. В `local`/`testing` он создаёт по одному явно техническому элементу `group_format` и `gender`, чтобы Stage 7 можно было пройти без ручных SQL-вставок. Это тестовые fixtures, а не утверждённые продуктовые значения; production seed их не создаёт. Цены размещения/продления по-прежнему не задаются.
...
Формы групп читают только активные элементы справочников `group_format` и `gender`. Обычный local/testing seed даёт явно тестовые варианты для ручной проверки; production seed намеренно оставляет эти справочники без элементов до утверждения продуктовых значений. Stage 7 не создаёт платежи и для платного тарифа создаёт группу сразу в `draft`.
...
После обычного local seed проверьте desktop и ширину около 390px:
...
7. удалите все временные группы и платежные safety-fixtures; local/testing элементы справочников создаются seed-ом и не требуют ручной вставки.
